...
// ─── Liquid Metal Blob ────────────────────────────────────────────────────────
// Morphing sphere with real-time vertex displacement → chrome/mercury effect
...
function LiquidMetalBlob({ mouseRef }) {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      // Layered sine-wave noise for organic liquid motion
      const n =
...
function Scene({ mouseRef }) {
  return (
    <>
      {/* Dramatic multi-colored lighting for chrome reflections */}
      ...
      {/* Gold-tinted environment for chrome metalness */}
      <Environment
        preset="studio"
        background={false}
        intensity={1.4}
      ...
      <div className="max-w-7xl mx-auto px-8 h-16 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center gap-2.5 group">
          <div className="w-6 h-6 rounded-full border border-[#C9A84C]/50 flex items-center justify-center transition-all duration-300 group-hover:border-[#C9A84C] group-hover:bg-[#C9A84C]/10">
            <span className="font-display text-[#C9A84C] text-[11px]">S</span>
          </div>
        ...
        <div className="hidden md:flex items-center gap-8">
          {links.map((l, i) => (
            <a key={i} href={l.href}
              className="font-sans text-[13px] text-neutral-500 hover:text-neutral-100 transition-colors duration-250 tracking-wide">
              {l.label}
            </a>
          ))}
        </div>
        ...
        {/* CTA */}
        <a href="#pricing"
          className="btn-sweep hidden md:inline-flex items-center gap-2 px-5 py-2 rounded-full border border-[#C9A84C]/35 text-[#C9A84C] font-sans text-[13px] hover:bg-[#C9A84C]/8 hover:border-[#C9A84C]/70 transition-all duration-350">
          Da 349€
...
export default function Hero({ isReady }) {
  ...
  return (
    ...
    >
      ...
      {/* Radial vignette — keeps sphere visible at center, fades edges */}
      ...
      <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2.5 mb-10 px-4 py-1.5 rounded-full
                     border border-[#C9A84C]/18 bg-[#C9A84C]/5"
        >
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400/80 animate-pulse shrink-0" />
          <span className="font-mono text-[10.5px] tracking-[0.32em] uppercase text-[#C9A84C]/70">
        ...
        <motion.h1
          ...
          initial="hidden"
          animate={isReady ? 'visible' : 'hidden'}
          className="font-display text-[clamp(3.4rem,8.8vw,8.2rem)] font-light leading-[1.03]
                     tracking-[-0.025em] text-neutral-100 mb-7"
        >
          {words.map((w, i) => (
            <motion.span key={i} variants={wordV}
              style={{ display: 'inline-block', marginRight: '0.26em' }}
              className={w.gold ? 'text-gold' : ''}
            >
              {w.text}
            </motion.span>
          ))}
        ...
        <motion.p
          ...
          initial="hidden"
          animate={isReady ? 'visible' : 'hidden'}
          className="font-sans text-[clamp(0.95rem,1.65vw,1.18rem)] font-light
                     text-neutral-500 max-w-[520px] mx-auto leading-[1.75] mb-12"
        >
          Siti web di livello premium a partire da{' '}
          <span className="text-[#C9A84C] font-normal">349€</span>.
          Animazioni cinematografiche, performance enterprise
          e design che converte — senza le tariffe delle grandi agenzie.
        </motion.p>
        ...
        >
          <a href="#pricing"
            className="btn-sweep group px-9 py-[14px] bg-[#C9A84C] text-[#080808]
                       font-sans font-medium text-[13.5px] tracking-wide rounded-full
                       transition-all duration-450 hover:bg-[#E8C97A]
                       hover:shadow-[0_0_55px_rgba(201,168,76,0.3)]">
            Inizia il tuo progetto
            <span className="ml-2 inline-block transition-transform duration-300 group-hover:translate-x-1">→</span>
          </a>
          <a href="#work"
            className="px-9 py-[14px] glass rounded-full font-sans font-light
                       text-[13.5px] text-neutral-400 tracking-wide
                       hover:text-neutral-100 hover:border-white/15 transition-all duration-400">
            Guarda i lavori
          </a>
        </motion.div>
        ...
        >
          {[
            { v: '60+', l: 'Progetti completati' },
            { v: '5.0★', l: 'Rating clienti' },
            { v: '< 7gg', l: 'Consegna media' },
          ].map((s, i) => (
